...
class tor_client
{
  public:
	...
	mini::string http_post(const mini::net::uri& url, const mini::string_ref data)
	{
		//
		// take out the parts to local variables.
		//
		...
		result = mini::net::http::client::post(host, port, path, data, *stream);
...
connect_again:
  ...
  if (post == false)
  {
	  content = tor.http_get(mini::net::uri(0 ? "http://duskgytldkxiuqc6.onion/fedpapers/federndx.htm" : url.c_str()));
  }
  if (post == true)
  {
	  content = tor.http_post(mini::net::uri(0 ? "http://duskgytldkxiuqc6.onion/fedpapers/federndx.htm" : url.c_str), data.c_str());
  }
